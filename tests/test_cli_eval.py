...
def test_runs_human_eval_task() -> None:
    fake_task = object()
    fake_grader = object()
    fake_log = SimpleNamespace(status="success", location="/tmp/human-eval.eval")
    ...
    with patch(
        ...
        with patch("inspect_ai.model.get_model", return_value=fake_grader):
            with patch(
                "evals.inspect.human_eval.navi_human_eval",
                return_value=fake_task,
            ):
                with patch("inspect_ai.eval", return_value=[fake_log]) as inspect_eval:
                    exit_code = run_inspect_eval("human-eval", limit=1)
...
def test_runs_agent_bench_os_task() -> None:
    fake_task = object()
    fake_grader = object()
    fake_log = SimpleNamespace(status="success", location="/tmp/agentbench-os.eval")
    ...
    with patch(
        ...
        with patch("inspect_ai.model.get_model", return_value=fake_grader):
            with patch(
                "evals.inspect.agent_bench_os.navi_agent_bench_os",
                return_value=fake_task,
            ):
                with patch("inspect_ai.eval", return_value=[fake_log]) as inspect_eval:
                    exit_code = run_inspect_eval("agentbench-os", limit=1)

    assert exit_code == 0
    assert inspect_eval.call_args.args == (fake_task,)


def test_runs_swe_bench_verified_task() -> None:
    fake_task = object()
    fake_grader = object()
    fake_log = SimpleNamespace(status="success", location="/tmp/swe-bench.eval")
    ...
    with patch(
        ...
        with patch("inspect_ai.model.get_model", return_value=fake_grader):
            with patch(
                "evals.inspect.swe_bench.navi_swe_bench_verified",
                return_value=fake_task,
            ):
                with patch("inspect_ai.eval", return_value=[fake_log]) as inspect_eval:
                    exit_code = run_inspect_eval("swe-bench-verified", limit=1)
